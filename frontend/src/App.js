...
//   const AUDIO_API_URL = "https://localhost:5000/upload";
...
//       await axios.post(AUDIO_API_URL, result);
//       setResults([...results, result]);
//       setAudioFile(null);
//     } catch (err) {
...
function App() {
  ...
  // Replace with your actual API endpoints
  const TEXT_API_URL = "http://localhost:5000/text";
  const AUDIO_API_URL = "http://localhost:5000/upload";
  ...
  const handleTextAnalysis = async () => {
    ...
    setError("");
    try {
      // Send text to Flask API for analysis
      const response = await axios.post(TEXT_API_URL, { text });
      const result = { type: "text", input: text, ...response.data };

      // Store the result in your results state
      setResults([...results, result]);
      setText("");
    } catch (err) {
      setError("Text analysis failed");
  ...
  const handleAudioAnalysis = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || err.message || "Audio analysis failed"
      );
    } finally {
      setLoading(false);
    }
